...
$prompts = @(
    @{
        text = "Crea un Custom Post Type llamado 'libro' con soporte para título, editor, thumbnail y campos personalizados. Registra la función en init."
        category = "wordpress"
        complexity = "medium"
    },
    @{
        text = "Crea un Custom Post Type llamado 'autor con soporte para título, editor, thumbnail y campos personalizados. Registra la función en init."
        category = "wordpress"
        complexity = "medium"
    },
    @{
        text = "Crea un Custom Post Type llamado 'editorial' con soporte para título, editor, thumbnail y campos personalizados. Registra la función en init."
        category = "wordpress"
        complexity = "medium"
    },
    @{
        text = "Crea una taxonomía jerárquica 'género' para el CPT 'libro'. Registra la función en init."
        category = "wordpress"
        complexity = "medium"
    },
    @{
        text = "Crea una taxonomía jerárquica 'país' para el CPT 'autor'. Registra la función en init."
        category = "wordpress"
        complexity = "medium"
    },
    @{
        text = "Añade una metabox 'Detalles del libro' con los campos: editorial, año, ISBN. Incluye funciones de renderizado y guardado con sanitización."
        category = "wordpress"
        complexity = "medium"
    }
)

$outDir = "C:\llama-benchmark\model_comparison_v5-engram"
...
function Test-ModelQuality {
    ...
    $outputFile = Join-Path $outDir "${modelName}_responses_v5-engram.json"
...
# --- Ejecución principal ---
Write-Host "===== COMPARACION DE CALIDAD Y RENDIMIENTO (V5 engram) =====" -ForegroundColor Yellow
Write-Host "Estructura: 6 x backend" -ForegroundColor Yellow
