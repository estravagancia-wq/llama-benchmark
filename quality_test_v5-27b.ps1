# quality_test_v5-27b.ps1
# Compara calidad y rendimiento entre modelos (usando la API)
# Intercala backend (4 preguntas: 2 medium + 2 high) y WordPress (2 high)
# max_tokens aumentado a 31862 para contexto más realista tras interacción prolongada

$prompts = @(
    @{
        text = "Escribe una funcion PHP que valide una direccion de email usando filter_var y tambien una validacion extra (dominio no numerico, longitud maxima 255)."
        category = "backend"
        complexity = "medium"
    },
    @{
        text = "Crea una clase PHP User con propiedades private: id, name, email, password. Incluye constructor, getters/setters, y un metodo validate() que devuelva true si email y password cumplen unos criterios basicos (email valido, password minimo 8 caracteres)."
        category = "backend"
        complexity = "medium"
    },
    @{
    ...
    @{
        text = "Implementa una funcion recursiva factorial($n) en PHP que maneje numeros negativos lanzando una excepcion. Ademas, escribe los test unitarios usando PHPUnit para esta funcion."
        category = "backend"
        complexity = "high"
    },
    @{
    ...
    @{
        text = "Escribe un snippet PHP que conecte a una base de datos MySQL usando PDO, ejecute una consulta SELECT con parametros seguros (prepared statement) y devuelva los resultados como un array asociativo. Incluye manejo de excepciones."
        category = "backend"
        complexity = "medium"
    }
)

$outDir = "C:\llama-benchmark\model_comparison_v5-27b"
...
function Test-ModelQuality {
    ...
    foreach ($item in $prompts) {
        ...
        Write-Host "  [$promptIndex/6] ($category/$complexity) Prompt: $($prompt.Substring(0, [Math]::Min(60, $prompt.Length)))..." -ForegroundColor Gray
    ...
    $outputFile = Join-Path $outDir "${modelName}_responses-27b.json"
...
Test-ModelQuality -modelName "qwen3.6-27b"
